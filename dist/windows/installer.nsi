...
Section "-hidden"
    ...
    ;Search if freeNexus is already installed.
...
Section $(inst_qbt_req) ;"freeNexus (required)"
  ...
  WriteRegStr HKLM "Software\freeNexus" "InstallLocation" "$INSTDIR"

  ; Write the uninstall keys for Windows
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "DisplayName" "freeNexus ${PROG_VERSION}"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "UninstallString" '"$INSTDIR\uninst.exe"'
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "DisplayIcon" '"$INSTDIR\freenexus.exe",0'
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "Publisher" "The freeNexus project"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "URLInfoAbout" "https://www.freenexus.org"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "DisplayVersion" "${PROG_VERSION}"
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "NoModify" 1
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "NoRepair" 1
  WriteUninstaller "uninst.exe"
  ${GetSize} "$INSTDIR" "/S=0K" $0 $1 $2
  IntFmt $0 "0x%08X" $0
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\freeNexus" "EstimatedSize" "$0"

  ; freeNexus ProgID
  WriteRegStr HKLM "Software\Classes\freeNexus" "" "freeNexus Torrent File"
  WriteRegStr HKLM "Software\Classes\freeNexus" "FriendlyTypeName" "freeNexus Torrent File"
  WriteRegStr HKLM "Software\Classes\freeNexus\shell" "" "open"
  WriteRegStr HKLM "Software\Classes\freeNexus\shell\open\command" "" '"$INSTDIR\freenexus.exe" "%1"'
  WriteRegStr HKLM "Software\Classes\freeNexus\DefaultIcon" "" '"$INSTDIR\freenexus.exe",1'
...
Section /o $(inst_dekstop) ;"Create Desktop Shortcut"
  ...
  CreateShortCut "$DESKTOP\freeNexus.lnk" "$INSTDIR\freenexus.exe"

SectionEnd

Section $(inst_startmenu) ;"Create Start Menu Shortcut"

  CreateDirectory "$SMPROGRAMS\freeNexus"
  CreateShortCut "$SMPROGRAMS\freeNexus\freeNexus.lnk" "$INSTDIR\freenexus.exe"
  CreateShortCut "$SMPROGRAMS\freeNexus\Uninstall.lnk" "$INSTDIR\uninst.exe"

SectionEnd

Section $(inst_torrent) ;"Open .torrent files with freeNexus"

  ReadRegStr $0 HKLM "Software\Classes\.torrent" ""

  StrCmp $0 "freeNexus" clear_errors 0
  ...
  WriteRegStr HKLM "Software\Classes\.torrent" "" "freeNexus"
...
Function inst_torrent_user
  ...
  WriteRegStr HKCU "Software\Classes\.torrent" "" "freeNexus"
  WriteRegStr HKCU "Software\Classes\.torrent" "Content Type" "application/x-bittorrent"

FunctionEnd

Section $(inst_magnet) ;"Open magnet links with freeNexus"
  ...
  WriteRegStr HKLM "Software\Classes\magnet\DefaultIcon" "" '"$INSTDIR\freenexus.exe",1'
  WriteRegStr HKLM "Software\Classes\magnet\shell" "" "open"
  WriteRegStr HKLM "Software\Classes\magnet\shell\open\command" "" '"$INSTDIR\freenexus.exe" "%1"'
...
Function inst_magnet_user
  ...
  WriteRegStr HKCU "Software\Classes\magnet\DefaultIcon" "" '"$INSTDIR\freenexus.exe",1'
  WriteRegStr HKCU "Software\Classes\magnet\shell" "" "open"
  WriteRegStr HKCU "Software\Classes\magnet\shell\open\command" "" '"$INSTDIR\freenexus.exe" "%1"'
...
Section $(inst_firewall)
  ...
  nsisFirewallW::AddAuthorizedApplication "$INSTDIR\freenexus.exe" "freeNexus"
...
Function check_instance
  ...
  check:
  FindProcDLL::FindProc "freenexus.exe"
...
Function PageFinishRun
  ...
  !insertmacro UAC_AsUser_ExecShell "" "$INSTDIR\freenexus.exe" "" "" ""
