...
bool upgrade(bool ask = true)
{
    ...
    SettingsPtr oldResumeSettings = Profile::instance().applicationSettings(QLatin1String("freeNexus-resume"));
...
{
    QSettings iniFile(QSettings::IniFormat, QSettings::UserScope, "freeNexus", application);
    if (!iniFile.allKeys().isEmpty()) return; // We copy the contents of plist, only if inifile does not exist(is empty).

    QSettings *plistFile = new QSettings("freeNexus", application);
...
{
    migratePlistToIni("freeNexus-data");
    migratePlistToIni("freeNexus-rss");
    migratePlistToIni("freeNexus");
}
#endif  // Q_OS_MAC
...
void migrateRSS()
{
    ...
    SettingsPtr qBTRSS = Profile::instance().applicationSettings(QLatin1String("freeNexus-rss-feeds"));
    if (!qBTRSS->allKeys().isEmpty()) return; // We move the contents of RSS old_items only if inifile does not exist (is empty).

    SettingsPtr qBTRSSLegacy = Profile::instance().applicationSettings(QLatin1String("freeNexus-rss"));
    QHash<QString, QVariant> allOldItems = qBTRSSLegacy->value("old_items", QHash<QString, QVariant>()).toHash();

    if (!allOldItems.empty()) {
        qDebug("Moving %d old items for feeds to freeNexus-rss-feeds", allOldItems.size());
