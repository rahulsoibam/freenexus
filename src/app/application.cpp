...
#endif
{
    ...
    Logger::instance()->addMessage(tr("freeNexus %1 started", "freeNexus v3.2.0alpha started").arg(QBT_VERSION));
}
...
void Application::sendNotificationEmail(const BitTorrent::TorrentHandle *torrent)
{
    ...
    const QString content = tr("Torrent name: %1").arg(torrent->name()) + "\n"
        ...
        + tr("Thank you for using freeNexus.") + "\n";
    ...
    smtp->sendMail(pref->getMailNotificationSender(),
                     ...
                     tr("[freeNexus] '%1' has finished downloading").arg(torrent->name()),
                     content);
}
...
int Application::exec(const QStringList &params)
{
    ...
    const QString mesg = QString("\n******** %1 ********\n").arg(tr("Information"))
        + tr("To control freeNexus, access the Web UI at %1")
...
void Application::initializeTranslation()
{
    ...
    if (m_translator.load(QLatin1String(":/lang/freenexus_") + localeStr))
