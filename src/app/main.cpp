...
int main(int argc, char *argv[])
{
    ...
    try {
        // Create Application
        QString appId = QLatin1String("freeNexus-") + Utils::Misc::getUserIDString();
        ...
        if (!qputenv("FREENEXUS", QBT_VERSION))
        ...
        // Check if freeNexus is already running for this user
        if (app->isRunning()) {
#ifdef DISABLE_GUI
            if (params.shouldDaemonize) {
                throw CommandLineParameterError(QObject::tr("You cannot use %1: freeNexus is already running for this user.")
                                     .arg(QLatin1String("-d (or --daemon)")));
            }
            else
#endif
            qDebug("freeNexus is already running for this user.");
        ...
        // For more info see:
        // 1. https://github.com/freenexus/freeNexus/issues/4209
...
#if !defined Q_OS_WIN && !defined Q_OS_HAIKU
    const char msg[] = "\n\n*************************************************************\n"
        "Please file a bug report at http://bug.freenexus.org and provide the following information:\n\n"
        "freeNexus version: " QBT_VERSION "\n\n"
        "Caught signal: ";
    reportToUser(msg);
...
bool userAgreesWithLegalNotice()
{
    ...
    const QString eula = QString("\n*** %1 ***\n").arg(QObject::tr("Legal Notice"))
        + QObject::tr("freeNexus is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.") + "\n\n"
...
#else
    QMessageBox msgBox;
    msgBox.setText(QObject::tr("freeNexus is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.\n\nNo further notices will be issued."));
