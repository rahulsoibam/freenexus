...
// See extensive discussion in https://github.com/freenexus/freeNexus/pull/3018
#ifndef Q_OS_MAC
...
bool Preferences::WinStartup() const
{
    ...
    return settings.contains("freeNexus");
}
...
void Preferences::setWinStartup(bool b)
{
    ...
    if (b) {
        ...
        settings.setValue("freeNexus", bin_path);
    }
    else {
        settings.remove("freeNexus");
    }
}
#endif
...
{
    return value("Preferences/MailNotification/sender", "freeNexus_notification@example.com").toString();
}
...
bool Preferences::isTorrentFileAssocSet()
{
    ...
    if (settings.value(".torrent/Default").toString() != "freeNexus") {
        qDebug(".torrent != freeNexus");
        return false;
    }
...
void Preferences::setTorrentFileAssoc(bool set)
{
    ...
    if (set) {
        ...
        if (!old_progid.isEmpty() && (old_progid != "freeNexus"))
            settings.setValue(".torrent/OpenWithProgids/" + old_progid, "");
        settings.setValue(".torrent/Default", "freeNexus");
    }
