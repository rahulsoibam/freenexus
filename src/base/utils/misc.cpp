...
void Utils::Misc::shutdownComputer(const ShutdownDialogAction &action)
{
    ...
    if (action == ShutdownDialogAction::Suspend) {
        ...
        const QString msg = QCoreApplication::translate("misc", "freeNexus will shutdown the computer now because all downloads are complete.");
...
void Utils::Misc::openFolderSelect(const QString &absolutePath)
{
    ...
    if ((output == "dolphin.desktop") || (output == "org.kde.dolphin.desktop"))
        ...
        // "caja" manager can't pinpoint the file, see: https://github.com/freenexus/freeNexus/issues/5003
        openPath(path.left(path.lastIndexOf("/")));
#else
    openPath(path.left(path.lastIndexOf("/")));
#endif
