...
SettingsStorage::SettingsStorage()
    : m_data{TransactionalSettings(QLatin1String("freeNexus")).read()}
    , m_dirty(false)
...
bool SettingsStorage::save()
{
    ...
    TransactionalSettings settings(QLatin1String("freeNexus"));
...
QVariantHash TransactionalSettings::read()
{
    ...
    if (!newPath.isEmpty()) { // "_new" file is NOT empty
        ...
        // in their final position. So assume that freenexus_new.ini/freenexus_new.conf
...
bool TransactionalSettings::write(const QVariantHash &data)
{
    ...
    // Write everything to freeNexus_new.ini/freeNexus_new.conf and if it succeeds
    // replace freeNexus.ini/freeNexus.conf with it.
