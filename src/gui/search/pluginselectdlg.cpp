...
void PluginSelectDlg::on_actionUninstall_triggered()
{
    ...
    if (error)
        QMessageBox::warning(this, tr("Uninstall warning"), tr("Some plugins could not be uninstalled because they are included in freeNexus. Only the ones you added yourself can be uninstalled.\nThose plugins were disabled."));
    else
...
void PluginSelectDlg::askForLocalPlugin()
{
    QStringList pathsList = QFileDialog::getOpenFileNames(
                ...
                tr("freeNexus search plugin") + QLatin1String(" (*.py)")
                );
